...
//player - opponent
int find_value(OthelloBoard board, int cur_player)
{
    /*
    if(((int)board.disc_count[1]+(int)board.disc_count[2])>40)
        return (((int)board.disc_count[1] - (int)board.disc_count[2])*1 + heuristic(board));
    else*/
...
{   
    ...
    {
        return find_value(board_cur, maxminizingPlayer);
    }
    ...
    {
       int value = INF;
       for(int d=0;d<board_cur.next_valid_spots.size();d++)
        {
          OthelloBoard board_next = board_cur;
          board_next.put_disc(board_cur.next_valid_spots[d]);
